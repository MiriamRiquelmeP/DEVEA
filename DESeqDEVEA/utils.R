...
customGO <- function(data, universe = NULL, species = "Hs", prior.prob = NULL,
    ...
    if(species=="At"){
        obj <- paste0("org.At.tairGO2ALLTAIRS")
    }else{
...
go2DT <- function(enrichdf, data, orderby = NULL, nrows = NULL) {
    ...
    if(nrow(enrichdf)==0){
        CAup <- data.frame("Term"="","Ont"="", "N"="","DEG"="","p-value"="","Ont.level"="","url"="")
    }else{
    names(data) <- c("SYMBOL", "ENTREZID")
    if(!is.null(orderby)){
    ...
    for (i in seq(1:length(CAup$genes))) {
       mg <- (unlist(strsplit(CAup$genes[i], ", ")))
       mg <- gsub(" ","",mg)
       mg2 <- match(mg, as.character(data$ENTREZID))
...
kegg2DT <- function(enrichdf, data, orderby = NULL, nrows = NULL) {
    ...
    if(nrow(enrichdf)==0){
        CAup <- data.frame("Pathway"="","N"="","DE"="","P.DE"="","genes"="","url"="")
    }else{
    names(data) <- c("SYMBOL", "ENTREZID")
    if(!is.null(orderby)){
...
# PCA de un objeto DESeq #####################
 ...
 if(length(intgroup)>1){
    p <- ggplot(data = d,
      ...
      geom_point(size = 3) +
      ggtitle(paste0("PCA for top ",min(ntop, length(rv))," variant genes on transformed data")) +
    ...
    p <- ggplot(data = d,
      ...
      geom_point(size = 3) +
      ggtitle("PCA for top 500 variant genes on transformed data") +
      xlab(paste0("PC",axes[1],": ", round(percentVar[axes[1]] * 100), "% variance")) +
      ylab(paste0("PC",axes[2],": ", round(percentVar[axes[2]] * 100), "% variance")) +
      scale_color_manual(values = colours, name = intgroup[1]) +
      #coord_fixed() +
...
getSigUpregulated <- function(dds, pval=0.05, logfc=0, specie="Mm"){
  ...
  if(specie=="At"){
    annot <- data.frame(SYMBOL = rk$GeneName_Symbol, ENTREZID = rk$ENSEMBL)# 07/02/2021
  }else{
...
geneIdConverter2 <- function(genes, specie="Mm"){
  ...
  if(specie=="Mm"){
    ensdb <- EnsDb.Mmusculus.v79
    orgdb <- org.Mm.eg.db
    regexp <-"^(ENSMUSG|ensmusg)"
    noregexp <- "^(?!ENSMUSG|ensmusg)"
  }else if(specie=="Hs"){
    ensdb <- EnsDb.Hsapiens.v86
    orgdb <- org.Hs.eg.db
    regexp <-"^(ENSG|ensg)"
    ...
    ensdb <- EnsDb.Rnorvegicus.v79
    orgdb <- org.Rn.eg.db
    regexp <-"^(ENSRNOG|ensrnog)"
    noregexp <- "^(?!ENSRNOG|ensrnog)"
  }else{
    ensdb <- ensembldb::EnsDb("./resources/At/EnsDb.Athaliana.v94.sqlite")
    orgdb <- org.At.tair.db
    regexp <-"^(AT.G|at.g)"
  ...
  ensrows <- grep(regexp, genes, perl=TRUE) # filas ensembl
  noensrow <- grep(noregexp, genes, perl=TRUE) # filas no ensembl
  annot$ENSEMBL <- NA
  annot$SYMBOL <- NA
  annot$ENTREZID <- NA
  ...
  if(length(ensrows) != 0){
        ...
        annot$SYMBOL[ensrows] <- mapIds(ensdb, keys = annot$genes[ensrows], column = "SYMBOL", keytype = "GENEID" )
        annot$SYMBOL[ensrows] <- mapIds(orgdb, keys = annot$genes[ensrows], column = "SYMBOL", keytype = "ENSEMBL" )
  }
  # si hay symbol
  if(length(noensrow)!=0){
    annot$ENSEMBL[noensrow] <- mapIds(ensdb, keys = annot$genes[noensrow], column = "GENEID", keytype = "SYMBOL" )
    ...
    result2_trycatch <- tryCatch(
      {mapIds(orgdb, keys = annot$genes[which(grepl(noregexp, genes, perl=TRUE) & is.na(annot$SYMBOL)) ],
              column = "SYMBOL", keytype = "ALIAS" )},
      error = function(e){return(NA)}
    )
    annot$SYMBOL[which(grepl(noregexp, genes, perl=TRUE) & is.na(annot$SYMBOL)) ] <- result2_trycatch
  }
  ...
  }else{ #esto es sólo para arabidopsis
      if(length(ensrows) != 0){
          annot$ENSEMBL[ensrows] <- as.character(annot$genes[ensrows])
          annot$description[ensrows] <- mapIds(orgdb, keys = annot$gene[ensrows], column = "GENENAME", keytype = "TAIR" )
          annot$SYMBOL[ensrows] <- mapIds(orgdb, keys = annot$genes[ensrows], column = "SYMBOL", keytype = "TAIR" )
          annot$ENTREZID[ensrows] <- mapIds(orgdb, keys = annot$genes[ensrows], column = "ENTREZID", keytype = "TAIR" )
      }
      if(length(noensrow)!=0){
          result_trycatch <- tryCatch( 
              { mapIds(orgdb, keys = annot$genes[noensrow], column = "TAIR", keytype = "SYMBOL" )
              } ,
              error = function(e){return(NA)}
          )
          annot$ENSEMBL[noensrow] <- result_trycatch
...
gseaKegg <- function(res, specie, gseadb){
 ...
 if(specie!="At"){ 
    ...
    attr(vectRank, "names") <- as.character(resRank$ENTREZ)
     ...
     resRank <- res.sh[!is.na(res.sh$ENSEMBL), c("ENSEMBL","log2FoldChange") ] # 07/02/2021
     vectRank <- resRank$log2FoldChange
     attr(vectRank, "names") <- as.character(resRank$ENSEMBL)
  }
  mygsea <- clusterProfiler::GSEA(vectRank, 
                                  TERM2GENE = pathwayDataSet, 
                                  by="fgsea", pvalueCutoff = 0.1)
  if(specie!="At"){
      gseasp <- paste0("org.",specie,".eg.db")
      mygsea <- DOSE::setReadable(mygsea, gseasp, "ENTREZID")
  }else{
      gseasp <- "org.At.tair.db"
      mygsea <- DOSE::setReadable(mygsea, gseasp, "TAIR")
  }
  return(mygsea)
}
...
heat2 <- function (vsd, n = 40, intgroup = NULL, sampleName = NULL,
  ...
  if(specie=="Mm"){
    ...
    if(!isTRUE(ggplt)){
      heatmaply(mat, labRow = labrow, col_side_colors = df, main = "Heatmap top variant gene expression on transformed data",
                ...
                margins = c(50,50,35,0)) }
    else{
      ggheatmap(mat, labRow = labrow, col_side_colors = df, main = "Heatmap top variant gene expression on transformed data",
                colors = viridis(n=256, alpha = 1, begin = 0, end = 1, option = "magma"),
              col_side_palette = ann, labCol = as.character(vsd[[sampleName]] ), fontsize_row = fsr,
              margins = c(50,50,35,0) )}
}
...
cluster <- function(vsd, intgroup = "condition", ggplt = FALSE)
  ...
  if(!isTRUE(ggplt)){
    heatmaply(sampleDistMatrix_vsd, colors = colors, margins = c(50,50,50,0), main = "Heatmap clustering of samples on transformed data"  )
  }else{
    ggheatmap(sampleDistMatrix_vsd, colors = colors, margins = c(50,50,50,0), main = "Heatmap clustering of samples on transformed data"  )  
    }
}
...
boxViolin <- function(datos=NULL, vsd=NULL, names=NULL, boxplotswitch=NULL,
    ...
    if(!isTRUE(boxplotswitch)){
        p <- p + geom_boxplot() + 
            ggtitle('Box plot for the transformed counts per gene') +
            theme(axis.text.x = element_text(angle = 90), plot.title = element_text(hjust = 0.5)) +
            scale_fill_manual( values = customColor )  
        return(p)
    } else {
        p <- p + geom_violin() + 
            ggtitle('Violin plot for the transformed counts per gene') +
            theme(axis.text.x = element_text(angle = 90), plot.title = element_text(hjust = 0.5))+
          scale_fill_manual( values = customColor )
        return(p)
        }
...
krtp <- function(res, specie="Mm", pval, fcdown, 
  ...
  if(specie=="At"){
      dfIdeoBanda <- NULL
      seqlevels(dfRanges) <- paste0("chr",seqlevels(dfRanges))
  }else{
