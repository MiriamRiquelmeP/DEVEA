...
gseaKegg <- function(res, specie, gseadb){
  ...
  if(specie!="At"){ 
      ...
      resRank <- res.sh[!is.na(res.sh$ENTREZID), c("ENTREZID","logFC") ] # 07/02/2021
      vectRank <- resRank$logFC
      attr(vectRank, "names") <- as.character(resRank$ENTREZ)
  }else{
      attr(vectRank, "names") <- as.character(res.sh$ENSEMBL)
      resRank <- res.sh[!is.na(res.sh$ENSEMBL), c("ENSEMBL","logFC") ] # 07/02/2021
      vectRank <- resRank$logFC
...
krtp <- function(res, specie="Mm", pval, fcdown, 
  ...
  if(annotation =="symbol"){genesv1 <- genes$SYMBOL}
