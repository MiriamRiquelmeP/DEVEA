    ...
    tabItem(tabName = "info",
            fluidRow(column(width = 4,
              box(width=12, 
                    pickerInput(
                          inputId = "specie",
                          label = "Select species",
                          choices = list( "Human" = "Hs", "Mouse" = "Mm", "Rat" = "Rn", "Arabidopsis" = "At"),
                          options = list(title = "species"),
                          selected = "Mm"
                      ),
                    pickerInput(
                      ...
                      choices = list("Ensembl/TAIR" = "ensg", "Symbol"="symbol"),
                      options = list(title="annotation"),
                      selected = "ensg"
                      ),
            ...
            box(width=12,
                ...
                p(HTML(paste0("Option 1. ", '<B>Gene list and statistical values (GL + SV) mode: </B>')), HTML(paste0("consists of a ", '<B>gene list (GL)</B>')),
                  ...
                  This table must be uploaded as a file, following the same order as the file mode.", 
                  br(),
                  br(),
                  ...
                  uploaded as a file or directly integrated in the application by copy/paste in the dedicated field. "),
                br(),
...
server <- function(input, output, session) {
      ...
      if(input$geneList!="" ){
        if (annotation() == "ensg") {
          if ( length(which(grepl("^ENS", input$geneList, ignore.case = TRUE))) < length(input$geneList) & 
               length(which(grepl("^AT", input$geneList, ignore.case = TRUE))) < length(input$geneList) ) {
            shinyalert("Oops!!", "One or more genes are not 
            in ENSEMBL/TAIR format",
                       type = "error")
          } else{
        ...
        if (annotation() == "symbol") {
          if (length(which(grepl("^ENS", input$geneList, ignore.case = TRUE))) ==  length(input$geneList) | 
            ...
            shinyalert("Oops!!", "Looks like this entry is 
                       ENSEMBL/TAIR please check your selection", 
                       type = "error")
          } else{
            validatedGene$list <- validateGeneList(input$geneList, specie(), annotation() )
            data$df <- formatData( validatedGene$list, specie(), annotation() )
      ...
      if(input$geneFile$datapath != "" ){
        ...
        if (annotation() == "ensg") {
          if(length(which(grepl("^ENS", validatedGene$list[,1], ignore.case = TRUE))) < nrow(validatedGene$list)  & 
             length(which(grepl("^AT", validatedGene$list[,1], ignore.case = TRUE))) < nrow(validatedGene$list) ) {
            shinyalert("Oops!!", "One or more genes are not 
            in ENSEMBL/TAIR format",
                       type = "error" )
          } else{
  ...
  observeEvent(input$enrichButtons,{
    if( dim(data$df)[2]==3 ){
      ...
      if(specie()=="At"){
      kgg$all <- customKegg(data$dfilt[,c("SYMBOL","ENSEMBL") ], species = specie(), universe = bckgnd$ENTREZID  )
      kggDT$all <- kegg2DT(kgg$all, data$dfilt[,c("SYMBOL","ENSEMBL") ] )
      go$all <- customGO(data$dfilt[,c("SYMBOL","ENSEMBL") ], species = specie(), universe = bckgnd$ENTREZID )
      goDT$all <- go2DT(enrichdf = go$all, data = data$dfilt[,c("SYMBOL","ENSEMBL") ] )
      }else{
...
## sidebar menu kegg ###################
  ...
  output$menuGSEA <- renderMenu({
    shiny::validate(need(go$all,""))
    shiny::validate(need(dim(data$df)[2]==5,""))  
      sidebarMenu(  
          menuItem("GSEA",
                   tabName = "gsea",
 ...
 # ....................... ####
  ...
  output$gseaSelectize <- renderUI({
    if(specie()=="Mm"){
      ...
      datasets <- list.files("./resources/At/GSEA/")
  ...
  output$gseaTable <- DT::renderDataTable(server=FALSE,{
    ...
    if(specie()!="At"){
        ...
        gsea$gsea <- gseaKegg(data$dfilt[, c("ENSEMBL","logFC")], specie(), gseadb = input$gseadb )
    }
    mygsea <- gsea$gsea
    ...
    table <- mygsea@result[mygsea@result$p.adjust<=0.05 ,2:10] %>% 
  ...
  output$gseaPlot <- renderPlot({
    ...
    if( length(which(mygsea@result$p.adjust<=0.05)) == 0 ){
      ...
      if(length(gseanr) == 1){
        p <- enrichplot::gseaplot2(gsea$gsea, geneSetID = gseanr, pvalue_table = TRUE, ES_geom = "line") + geom_vline(aes_(xintercept = mygsea@result$rank[gseanr]), linetype = "longdash", color = "red", linewidth = 1.5)
        svg$gseaplot <- p
        print(p)
      } else{
